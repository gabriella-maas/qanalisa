# QA Assistant — Design Specification

**Date:** 2026-09-17  
**Status:** Draft approved in conversation; awaiting final spec review  
...
- OpenAI-based analysis through a provider abstraction.
...
Defines an interface independent of OpenAI. OpenAI is the first implementation. Future providers or local models should be replaceable without changing the core analysis pipeline.
...
OpenAI API usage is billed separately from ChatGPT subscriptions, so the MVP should avoid unnecessary repeat calls.
...
- parses `--reanalyze`;
- parses `--test CT04`;
- parses `qa bug FN-1234`;
- parses `qa bug FN-1234 --full`;
...
5. **Provider-independent AI boundary.** OpenAI is the first provider, not a hard-coded architectural dependency.
